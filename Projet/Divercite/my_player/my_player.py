...
class MyPlayer(PlayerDivercite):
    ...
    def compute_action(self, current_state: GameState, remaining_time: int = 1e9, **kwargs) -> Action:
        ...
        for depth in range(2, max_depth + 1):
            score, action_candidate = self.minimax_with_tt(
    ...
    def minimax_with_tt(self, current_state: GameStateDivercite, depth: int, alpha: float, beta: float,
        ...
        if maximizing_player:
            ...
            for action in possible_actions:
                next_state = action.get_next_game_state()
                eval_score, _ = self.minimax_with_tt(next_state, depth - 1, alpha, beta, False, start_time, time_limit)
                if eval_score > value:
                    value = eval_score
                    best_action = action
            ...
            for action in possible_actions:
                next_state = action.get_next_game_state()
                eval_score, _ = self.minimax_with_tt(next_state, depth - 1, alpha, beta, True, start_time, time_limit)
                if eval_score < value:
                    value = eval_score
                    best_action = action
                beta = min(beta, value)
